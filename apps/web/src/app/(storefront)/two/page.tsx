...
export default function PocTwoPage() {
  return (
    <div className="min-h-screen bg-white text-[#0A0A0A] font-sans">
      ...
      {/* Header */}
      <header className="flex items-center justify-between px-4 sm:px-8 py-3 sm:py-5 border-b border-gray-100 sticky top-0 z-40 bg-white">
        <div className="flex items-center gap-2">
          <img src="/logo.png" alt="TSE Online" style={{ width: 140, height: 36, objectFit: 'contain' }} className="sm:w-[200px] sm:h-[48px]" />
          <span className="hidden sm:inline text-xs text-gray-400 font-medium tracking-wide -ml-2">Cartridges</span>
        </div>
        ...
        <button className="bg-[#0A0A0A] text-white text-xs sm:text-sm font-black px-4 sm:px-5 py-2 sm:py-2.5 rounded-xl cursor-pointer hover:bg-gray-800 transition-colors">
          Shop Now
        </button>
      </header>

      {/* Hero — oversized editorial, light */}
      <section className="pt-10 sm:pt-14 pb-0 overflow-hidden border-b border-gray-100 relative">
        ...
        <div className="relative z-10 overflow-hidden px-4 sm:px-8">
          <h1
            className="font-black uppercase text-[#0A0A0A] leading-none"
            style={{ fontSize: 'clamp(2.8rem, 12vw, 8rem)', lineHeight: 0.85, letterSpacing: '-0.04em' }}
          >
            QUALITY
          </h1>
        </div>
        <div className="relative z-10 overflow-hidden">
          <h1
            className="font-black uppercase text-[#0D9488] leading-none pl-4 sm:pl-8"
            style={{ fontSize: 'clamp(2.8rem, 12vw, 8rem)', lineHeight: 0.85, letterSpacing: '-0.04em' }}
          >
            GENERICS.
          </h1>
        ...
        <div className="relative z-10 px-4 sm:px-8 mt-6 sm:mt-10 pb-10 sm:pb-14 max-w-2xl">
          <p className="text-base sm:text-xl text-gray-500 leading-relaxed font-medium mb-5 sm:mb-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <button className="w-full sm:w-auto shrink-0 whitespace-nowrap bg-[#0D9488] hover:bg-[#0f766e] text-white font-black px-6 sm:px-7 py-3.5 sm:py-4 rounded-2xl cursor-pointer transition-colors border-2 border-[#0D9488] hover:border-[#0f766e] text-sm sm:text-base">
              Shop All Cartridges
            </button>
            <button className="w-full sm:w-auto shrink-0 whitespace-nowrap border-2 border-[#0D9488] hover:border-[#0f766e] text-[#0D9488] hover:text-[#0f766e] font-bold px-6 sm:px-7 py-3.5 sm:py-4 rounded-2xl cursor-pointer transition-colors text-sm sm:text-base">
              Find My Cartridge
            </button>
          </div>
        </div>
      ...
      <div className="border-b border-gray-100 py-3 sm:py-4 overflow-hidden bg-gray-50">
        <div className="flex animate-ticker whitespace-nowrap">
          {marquee.map((brand, i) => (
            <span key={i} className="inline-flex items-center gap-4 sm:gap-5 px-4 sm:px-6 text-xs font-black text-gray-400 hover:text-gray-700 cursor-pointer transition-colors uppercase tracking-widest">
              {brand}
      ...
      <div className="border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-8 py-6 sm:py-8 grid grid-cols-2 md:grid-cols-4 gap-5 sm:gap-8">
          {[
            ...
            <div key={s.label}>
              <div className="text-2xl sm:text-4xl font-black text-[#0A0A0A] tracking-tight leading-none">{s.value}</div>
              <div className="text-gray-400 text-xs mt-1.5 sm:mt-2 leading-snug">{s.label}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-8 py-10 sm:py-16">
        ...
        <div className="divide-y divide-gray-100">
          {products.map((p, i) => (
            <div key={p.sku} className="flex items-center justify-between py-4 sm:py-5 group cursor-pointer hover:bg-gray-50 -mx-2 sm:-mx-4 px-2 sm:px-4 rounded-2xl transition-colors">
              <div className="flex items-center gap-3 sm:gap-6 min-w-0">
                <span className="text-gray-300 text-sm font-mono w-5 shrink-0">{String(i + 1).padStart(2, '0')}</span>
                <div className="min-w-0">
                  <div className="text-gray-900 font-semibold text-sm sm:text-base group-hover:text-[#0D9488] transition-colors truncate">{p.name}</div>
              ...
              <div className="flex items-center gap-2 sm:gap-5 shrink-0 ml-2">
                {p.save && <span className="text-xs font-bold text-green-600 hidden lg:block">{p.save}</span>}
                <span className="text-lg sm:text-xl font-black text-gray-900">{p.price}</span>
                <button className="bg-gray-900 hover:bg-[#0D9488] text-white text-xs font-bold px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl cursor-pointer transition-colors">
                  Add
                </button>
              </div>
            </div>
      ...
      <section className="bg-[#0D9488]">
        <div className="max-w-7xl mx-auto px-4 sm:px-8 py-10 sm:py-16 flex flex-col md:flex-row items-start md:items-center gap-6 sm:gap-8 justify-between">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-8 py-10 sm:py-16">
        <div className="mb-7 sm:mb-10">
          ...
          <h2 className="text-2xl sm:text-3xl font-black mt-1 tracking-tight">What customers say.</h2>
        </div>
        <div className="columns-1 sm:columns-2 gap-5 space-y-5">
          {reviews.map((r) => (
            <div key={r.name} className="break-inside-avoid bg-gray-50 border border-gray-100 rounded-3xl p-5 sm:p-6">
      ...
      {/* Footer CTA */}
      <div className="border-t border-gray-100 bg-[#0A0A0A] px-4 sm:px-8 py-10 sm:py-12">
