...
export type Category = {
   ...
   *
   * These cover the category layer only. The full cutover map — all 832 indexed
   * legacy URLs, products and model landing pages included — is generated by
   * `scripts/build-legacy-redirects.ts` into the nginx map that actually serves
   * the 301s. The 34 paths recorded here are asserted against that generator, so
   * a category can never be live here and missing from the redirect map.
   *
   * (The 2026-08-21 note said this had to wait on a Search Console indexed-URL
   * export, because the legacy sitemap omitted its own `/product-category/*`
   * pages. AIOSEO publishes them now, so the crawl alone is sufficient.)
   */
  legacyPaths: string[]
}
...
export const CATEGORIES: readonly Category[] = [
  ...
  },
  {
    ...
    legacyPaths: ['/product-category/pantum-cartridges', '/the-best-quality-generic-pantum-cartridges-satisfaction-guaranteed', '/the-best-pantum-cartridges-to-enhance-your-printing-experience'],
  },
  {
